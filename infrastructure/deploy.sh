...
if gsutil ls gs://$PRIMARY_BUCKET/**/*.html >/dev/null 2>&1; then
    gsutil -m setmeta -h "Content-Type:text/html" gs://$PRIMARY_BUCKET/**/*.html 2>&1 | grep -v "multiprocessing on MacOS" | grep -v "bugs.python.org"
    gsutil -m setmeta -h "Content-Type:text/html" gs://$SECONDARY_BUCKET/**/*.html 2>&1 | grep -v "multiprocessing on MacOS" | grep -v "bugs.python.org"
fi

# Only set CSS content types if CSS files exist
if gsutil ls gs://$PRIMARY_BUCKET/**/*.css >/dev/null 2>&1; then
    gsutil -m setmeta -h "Content-Type:text/css" gs://$PRIMARY_BUCKET/**/*.css 2>&1 | grep -v "multiprocessing on MacOS" | grep -v "bugs.python.org"
    gsutil -m setmeta -h "Content-Type:text/css" gs://$SECONDARY_BUCKET/**/*.css 2>&1 | grep -v "multiprocessing on MacOS" | grep -v "bugs.python.org"
fi

# Only set JS content types if JS files exist
if gsutil ls gs://$PRIMARY_BUCKET/**/*.js >/dev/null 2>&1; then
    gsutil -m setmeta -h "Content-Type:application/javascript" gs://$PRIMARY_BUCKET/**/*.js 2>&1 | grep -v "multiprocessing on MacOS" | grep -v "bugs.python.org"
    gsutil -m setmeta -h "Content-Type:application/javascript" gs://$SECONDARY_BUCKET/**/*.js 2>&1 | grep -v "multiprocessing on MacOS" | grep -v "bugs.python.org"
fi
